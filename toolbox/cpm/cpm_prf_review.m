function varargout = cpm_prf_review(varargin)
    ...
    %
    % cpm_prf_review(PRF,'generate_images',SPM)
    %
    % PRF - model
    % SPM - SPM structure
    %
    ...
    %
    % cpm_prf_review(PRF,'view_comparison',filename)
    %
    % PRF - PRF
    ...
    % ---------------------------------------------------------------------
    % Changes to fit with CPM framework by Simon R. Steinkamp
...
function review_prf_map(PRF, xY, fname, is_model_parameter)
    ...
    if nargin < 3 || is_model_parameter
        ...
        if isfield(PRF, 'pimg') && (exist(fullfile(PRF.dir, PRF.pimg), ...
                                          'file') > 0) && ...
    ...
    % Axes for plots
    ax = uipanel('Units', 'normalized', 'Position', [0 0 1 0.55], ...
                 'BackgroundColor', [1 1 1], 'BorderWidth', 0);
...
function review_single_prf(PRF, idx, parent)
    ...
    xy = PRF.U(1).gridpoints;

    if ndims(PRF.U(1).signals) > 2
        error("Not implemented for grids with n > 2");
    end
    [b1, b2] = size(PRF.U(1).signals);

    % PRF prior response
    z_prior  = feval(PRF.M.IS, PRF.M.pE{idx}, PRF.M, PRF.U, 'get_response', xy);
    z_prior  = reshape(z_prior, b1, b2);
    ...
    [x, x, prior_pd, post_pd] = spm_cpm_get_ppd(PRF_tmp, xy, idx, 1000);
    prior_pd = reshape(prior_pd, b1, b2);
    post_pd  = reshape(post_pd, b1, b2);
    clear PRF_tmp;
    ...
    if true
        ...
        [spE, sEp] = spm_cpm_get_ppd(PRF, xy, idx, 1000);
    ...
    x_tick = sort(unique(PRF.U(1).gridpoints(:, 1)))';

    if b1 == 1 || b2 == 1
        plotfunc = @plot;
        max_val  = max([max(z_prior(:)), max(prior_pd(:)), max(z_post(:)), max(post_pd(:))]);
        y_tick = linspace(0, max_val, 10)';
    else
        plotfunc = @imagesc;
        y_tick = sort(unique(PRF.U(1).gridpoints(:, 2)))';
    end

    x_tick = round(x_tick, 2);
    y_tick = round(y_tick, 2);

    x_labels = {};
    for xt = 1:length(x_tick)
        x_labels{xt} = num2str(x_tick(xt));
    end

    y_labels = {};
    for xt = 1:length(y_tick)
        y_labels{xt} = num2str(y_tick(xt));
    end

    x_tick = 1:3:length(x_tick);
    y_tick = 1:3:length(y_tick);
    ...
    for i = 1:num_plots
        ...
        set(gca, 'YDir', 'normal', 'XTick', x_tick', 'YTick', y_tick');

        set(gca, 'XTickLabel', x_labels(x_tick), 'YTickLabel', y_labels(y_tick));
    ...
    hpE        = spm_plot_quartiles(spE', x - jitter, 'Color', [0.6 0.6 0.6]);
    hold on;
    ...
    plot(x_response, PRF.y + PRF.R, 'Color', [0.3 0.3 0.3], 'LineStyle', '--');
    hold on;
