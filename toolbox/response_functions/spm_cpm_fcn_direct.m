function varargout = spm_cpm_fcn_direct(P, M, U, varargin)
    % Directly estimates response for a given input function.
    %
        ...
        switch action
            ...
            case 'latent_parameters'
                ...
                for pn = 1:length(params)
                    P.(params{pn}) = latent_parameter(P.(params{pn}), ...
                                                      M.pmin.(params{pn}), ...
                                                      M.pmax.(params{pn}));
                end
...
function [z, y, Z] = integrate_model(P, M, U)
    ...
    signal = U(1).model(P, U(1).fixedparams, U(1).data);
...
function P = correct_parameters(P, M)
    ...
    for pn = 1:length(params)
        P.(params{pn}) = constrain_parameter(P.(params{pn}), ...
                                             M.pmin.(params{pn}), ...
                                             M.pmax.(params{pn}));
    end
